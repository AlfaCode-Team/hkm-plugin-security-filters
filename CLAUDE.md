# SecurityFilters — Plugin Context for Claude

> Plugin for the **AlfacodeTeam PhpServicePlatform** (Sentinel) kernel.
> Package `alfacode-team/hkm-plugin-security-filters` · namespace `Plugins\SecurityFilters\` · solves `http.security_filters`
>
...
| | |
|---|---|
| `solves` | `http.security_filters` |
| `requires` | — |
| `exposes` | — |
| `emits` | — |
| Activation | global hooks + route filters — `Provider::boot()` registers CORS and SecureHeaders as pipeline hooks (every request) and publishes the `auth` / `throttle` / `hmac` / `shield` filter aliases (opt-in per route) |
...
`config[]` is also what `hkm plugins enable security-filters` seeds into the project's
...
|---|---|---|---|
| `HMAC_PROTECTED_PREFIX` | string | no | — |
| `HMAC_MAX_SKEW` | int | no | — |
| `REQUEST_SIGNING_SECRET` | string | no | — |
| `RATE_LIMIT_PREFIX` | string | no | — |
| `RATE_LIMIT_MAX` | int | no | — |
| `RATE_LIMIT_WINDOW` | int | no | — |
| `SHIELD_RULES` | string | no | — |
| `AUTH_PROTECTED_PATHS` | string | no | — |
| `CORS_ALLOWED_ORIGINS` | string | no | — |
| `CORS_ALLOWED_METHODS` | string | no | — |
| `CORS_ALLOWED_HEADERS` | string | no | — |
| `CORS_EXPOSED_HEADERS` | string | no | — |
| `CORS_MAX_AGE` | int | no | — |
| `CORS_ALLOW_CREDENTIALS` | bool | no | — |
| `HSTS_MAX_AGE` | int | no | — |
| `CONTENT_SECURITY_POLICY` | string | no | — |
...
The cross-cutting HTTP security stages, wired through **two different
mechanisms** — and a stage uses exactly ONE of them, never both.

### Global hooks (run on EVERY request, registered in `Provider::boot()`)

| Stage | Slot | Config |
|---|---|---|
| `CorsStage` | `after.security` | `CORS_ALLOWED_ORIGINS/METHODS/HEADERS`, `CORS_EXPOSED_HEADERS`, `CORS_ALLOW_CREDENTIALS`, `CORS_MAX_AGE` |
| `SecureHeadersStage` | `after.execute` | `CONTENT_SECURITY_POLICY`, `HSTS_MAX_AGE` |

### Route-filter aliases (opt-in per route via `"filters": [...]`)

| Alias | Stage | Config |
|---|---|---|
| `auth` | `RequireAuthStage` | also honours `AUTH_PROTECTED_PATHS` (exact / `prefix/*` / `*`) |
| `throttle:max,minutes` | `ApiRateLimitStage` | `RATE_LIMIT_PREFIX/MAX/WINDOW`; uses `CachePort` |
...
```jsonc
// a consuming plugin's module.json — declarative, no PHP
...
```
✗ Registering the SAME stage as BOTH a global hook AND a route filter — it then
  runs TWICE per request. For the rate limiter that means every request costs two
  counter increments and the effective limit is halved.
```

`RequireAuthStage` deliberately enforces when **either** the route declared the
`auth` filter **or** the path matches `AUTH_PROTECTED_PATHS`. The auth
SecurityLayer attaches `Identity` globally; this stage decides which routes
*demand* one.

`FilterRegistry` is shared, not owned by this plugin — any plugin may publish an
alias from its own `boot()` via `$http->filter('json', RequireJsonStage::class)`.
Two plugins binding the same alias to different stages throws.

## `hmac` VS `signed` — DIFFERENT THINGS

`hmac` authenticates a **machine-to-machine REQUEST**. `signed` (the kernel's
`UrlGenerator::signedRoute()`) authenticates a **LINK you email to a person**.
Reaching for the wrong one produces something that looks secure and is not.

## CORS AND SECURE HEADERS BELONG HERE, NOT IN THE KERNEL

The kernel has no CORS class by design. Do not propose adding one.

```
✗ `CORS_ALLOWED_ORIGINS=*` together with `CORS_ALLOW_CREDENTIALS=true` — the
  browser rejects it, and the intent behind it is almost always wrong
✗ Rate limiting on an InMemoryCache — under PHP-FPM each request is a new process,
  so the counter is always 1 and the limiter does nothing
```
